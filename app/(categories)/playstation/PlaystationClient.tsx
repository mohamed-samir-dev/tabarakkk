    ...
    fetch(`/api/products`)
      ...
      .then((data: Product[]) => {
        const filtered = data.filter((p) =>
          PS_CATEGORIES.includes(p.category ?? "")
        );
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-6xl mx-auto px-3 sm:px-4 py-3 sm:py-4">
          <div className="flex items-center gap-1.5 text-xs sm:text-sm text-[#1F6F8B] mb-2">
            ...
            <span>/</span>
            <span className="text-[#0F4C6E] font-medium">أجهزة بلاي ستيشن</span>
          </div>
          <h1 className="text-lg sm:text-2xl font-bold text-[#0a3550]">أجهزة بلاي ستيشن</h1>
